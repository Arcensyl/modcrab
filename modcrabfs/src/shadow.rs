...
use nix::{
    ...
    sys::{
        stat::{self, fchmodat, fstatat, utimensat, FchmodatFlags, Mode, UtimensatFlags},
        time::TimeSpec,
    },
    unistd::{close, fchownat, ftruncate, Gid, Uid},
};
use tap::prelude::*;

use crate::{
    filesystem::{mode_to_filetype, TTL},
    libc_extras::io_to_libc_error,
};
...
impl ShadowedDirectory {
    ...
    pub fn new(path: impl AsRef<Path>) -> LowResult<Self> {
        ...
        let handle = open(&path, OFlag::O_DIRECTORY | OFlag::O_RDONLY, Mode::empty())
            .map_err(|e| e as i32)?;
    ...
    pub fn truncate(&self, path: impl AsRef<Path>, size: u64) -> LowResult<()> {
        ...
        unsafe { ftruncate(OwnedFd::from_raw_fd(raw_fh), size as i64).map_err(|e| e as i32) }
    }
}

impl Drop for ShadowedDirectory {
    fn drop(&mut self) {
        close(self.handle).expect("Failed to close shadowed directory's file handle!");
    }
}
