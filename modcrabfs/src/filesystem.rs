...
impl ModcrabFS {
	...
	pub fn new(mountpoint: impl AsRef<Path>, cache: impl AsRef<Path>, mut overlay: Vec<PathBuf>) -> io::Result<Self> {
		let surface = overlay.pop()
		...
		let fs = Self {
			...
			shadowed: ShadowedDirectory::new(mountpoint).map_err(|e| io::Error::from_raw_os_error(e))?,
		};
	...
	pub fn mount(self) -> io::Result<()> {
		...
		let args = ["fsname=modcrabfs"].map(|s| OsStr::new(s));
	...
	fn register_path(&self, path: impl AsRef<Path>) -> io::Result<()> {
		...
		let virt = real.strip_prefix(&self.surface)
...
impl FilesystemMT for ModcrabFS {
    ...
    fn getattr(&self, _req: RequestInfo, path: &Path, fh: Option<u64>) -> ResultEntry {
		...
		if let Ok(real) = self.real_path(&path) {
			if self.is_shadowing(&real) {
				return Ok(self.shadowed.stat(&real)?);
			}
		}
    ...
    fn chmod(&self, _req: RequestInfo, path: &Path, fh: Option<u64>, mode: u32) -> ResultEmpty {
        ...
        let result = if let Some(fh) = fh {
			...
			let real = self.real_path(path).map_err(|e| e.raw_os_error().unwrap())?;
    ...
    fn chown(&self, _req: RequestInfo, path: &Path, fh: Option<u64>, uid: Option<u32>, gid: Option<u32>) -> ResultEmpty {
        let unwrapped_uid = uid.clone().unwrap_or(::std::u32::MAX);   // docs say "-1", but uid_t is unsigned
        let unwrapped_gid = gid.clone().unwrap_or(::std::u32::MAX);   // ditto for gid_t
        ...
        if -1 == result {
            let e = io::Error::last_os_error();
            error!("chown({:?}, {}, {}): {}", path, unwrapped_uid, unwrapped_gid, e);
            Err(e.raw_os_error().unwrap())
        } else {
            Ok(())
        }
    ...
    fn mknod(&self, _req: RequestInfo, parent_path: &Path, name: &OsStr, mode: u32, rdev: u32) -> ResultEntry {
        ...
        let real = self.real_path(&parent_path.join(name)).map_err(io_to_libc_error)?;  
        let result = unsafe {
    ...
    fn unlink(&self, _req: RequestInfo, parent_path: &Path, name: &OsStr) -> ResultEmpty {
		...
		self.transform(VirtualFileTransformation::Deletion { target: virt.to_path_buf() })
			.map_err(io_to_libc_error)
    }
    ...
    fn symlink(&self, _req: RequestInfo, parent_path: &Path, name: &OsStr, target: &Path) -> ResultEntry {
        ...
        match ::std::os::unix::fs::symlink(target, &real) {
            Ok(()) => {
				if !self.contains(&virt) { self.register_path(&real).map_err(io_to_libc_error)?; }
    ...
    fn rename(&self, _req: RequestInfo, parent_path: &Path, name: &OsStr, newparent_path: &Path, newname: &OsStr) -> ResultEmpty {
		...
		self.transform(VirtualFileTransformation::Relocation { from: virt.to_path_buf(), to: new_virt.to_path_buf() })
			.map_err(io_to_libc_error)
    }
    ...
    fn link(&self, _req: RequestInfo, path: &Path, newparent: &Path, newname: &OsStr) -> ResultEntry {
        ...
        match fs::hard_link(&real, &newreal) {
            Ok(()) => {
				if !self.contains(&newvirt) { self.register_path(&real).map_err(io_to_libc_error)?; }
