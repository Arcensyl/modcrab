...
use petgraph::{
    algo::has_path_connecting, graph::NodeIndex, stable_graph::StableDiGraph, visit::EdgeRef,
};
use rand::{thread_rng, Rng};

use crate::libc_wrappers;
use crate::{
    filesystem::{mode_to_filetype, stat_to_fuse, TTL},
    libc_extras::libc,
};
...
impl From<VirtualFileData> for DirectoryEntry {
    fn from(val: VirtualFileData) -> Self {
        DirectoryEntry {
            name: val.path.file_name().unwrap().to_os_string(),
...
fn query_file_type(path: impl AsRef<Path>) -> io::Result<FileType> {
    ...
    let stat = libc_wrappers::lstat(path).map_err(io::Error::from_raw_os_error)?;
...
fn read_dir(path: impl AsRef<Path>) -> io::Result<Vec<VirtualFileData>> {
    ...
    for entry in fs::read_dir(path)? {
        ...
        full_paths.insert(
            entry_path.file_name().unwrap().to_os_string(),
    ...
    let final_contents = entries
        .into_iter()
        .filter(|e| !(&e.name == "." || &e.name == ".."))
        .map(|e| VirtualFileData {
            path: full_paths.remove(&e.name).unwrap(),
            kind: e.kind,
            is_root: false,
        })
        .collect();
...
impl VirtualFileTree {
        ...
        'outer: loop {
            while let Some(item) = dirs[depth].pop() {
                if item.kind == FileType::Directory {
                    ...
                    self.update_child(roots[depth], item);
                }
            }
    ...
    fn update_child(&mut self, parent: NodeIndex, weight: VirtualFileData) -> NodeIndex {
        let link = weight.path.file_name().unwrap().to_ascii_lowercase();
    ...
    fn clear_orphans(&mut self) {
        ...
        self.graph
            .retain_nodes(|graph, node| has_path_connecting(&*graph, root, node, None));
    }
}
